# Character Development Methodology — DRAFT Ideas

> **Editorial Note — Source Material, Not Required Reading.** This file is raw source-mining material: a
> direct extraction and synthesis of published writing-craft books (Truby, Kress, Corbett, Boutros, St. John,
> Maisel, Alderson & Rosenfeld, Weiland, Bell, Card, Lisle, Swain, and Seger), one tier below the distilled
> Stage 00-05 Pipeline in this same `Character_Creation_Methodology/` folder (see the sibling `Pipeline/`
> directory). It is kept here for reference depth — nuance, alternate framings, and fuller technique detail —
> that the distilled Pipeline may have compressed away in the course of synthesis. Reading this file is **not**
> required to use the Pipeline itself; it is deeper background material for whoever wants it. Any
> project-specific named examples that appear in the body below are illustrative only, not required content,
> and have been left in place exactly as originally written.

**What this is:** a running scratchpad for *what needs to be in* a future "Character Development
Methodology" — a repeatable instruction sheet for producing genuine psychological depth and inner personal
conflict for any given character, distinct from the existing fill-in templates
(`Character_Spec_Fill-In_Sheet_Template.md`, `Companion_and_Romance_Questline_Fill-In_Sheet_Template.md`),
which capture *what fields* to fill in but not *how* to generate something genuinely deep for them.

**Status: pure brainstorm, not yet organized into an actual methodology.** Record candidate mechanisms and
ideas here as they come up, in whatever order they arrive. Sorting these into a real structured document —
deciding what's a pillar, what's a check, what's a worked example — is deliberately deferred until this list
has enough material to organize.

**Confirmed gap, checked directly (2026-08-08):** no character-development/psychological-depth methodology
currently exists anywhere in the repo. The closest precedent is `Forbidden_Trait_Design_Method.md`, which
derives forbidden traits from three existing inputs (Enneagram personality, personal history, personal
sensibilities) rather than inventing new criteria per character — a real, reusable discipline pattern, but
scoped narrowly to one mechanic (romance dealbreaker traits), not general inner-conflict generation.
...
given branch of a character's arc, never by a fixed percentage of anything. "The Midpoint" isn't a timestamp at the
...
always going to need to hold all of them at once, for every companion, simultaneously. A character's Lie, Want,
...
- **Enneagram-Rooted Contradiction.** Every companion already has a type/wing/instinct assignment. Inner
  conflict should be *derived* from that type's own structural core-fear/core-desire tension, never invented
  independently of it — e.g. a 5w6's conflict is never generic "trust issues," it's specifically the pull
  between withdrawal-for-safety (5) and loyalty-anxiety (6 wing). Makes the conflict load-bearing on
  something already established, not decoration bolted on afterward.

- **The Robot-Universal Friction Bank.** Now that `Robot_Universals` (the reference book at
  `TepenianUniverseTimeline/Reference/Robot_Universals/`) exists, there's a real menu of grounded,
  robot-specific friction points to draw from — none of which were usable material before that book existed:
  - the seeded-vs-emergent Personality Module tension (Robot Universals Ch.5)
  - city/locality belonging pulling against personal desire or path (Ch.13)
  - a mentor/mentee bond lost, betrayed, or never had (Ch.11)
  - an outlier Gen/Mark identity — a robot whose generation doesn't match her own city or peer group (Ch.14)
  - what the trust-bond stakes of romance actually cost a robot specifically, not generic jealousy (Ch.10, Ch.17)
  - a vulnerability tied to the real, narrow definition of integrity/death (Ch.4)
  Pick one or two per character, not all of them — specificity is the point, not a checklist every character runs
  through identically.

- **The Anti-Stereotype / Round-vs-Flat Check.** Already a labeled field in the Character Spec sheet, but not
  yet a real discipline step. Formalize it as an actual pass: does this conflict read as *this specific
  person's* problem, or could it be swapped onto any character of the same Enneagram type without anyone
  noticing? If the latter, it isn't done yet.

- **The Branch-Justification Check.** Ties to `Companion_System.md`'s existing Dual-Outcome Companion Perk
  law (2-5 mutually exclusive perks/questline, genuine trade-offs). The inner conflict should be *why* a
  companion's questline branches diverge the way they do — not a flat character with game-mechanical
  branches bolted on after the fact, unconnected to anything she actually struggles with internally.

*(Add new candidate mechanisms below this line as they come up. No need to organize them yet.)*

## From *The Anatomy of Story* — John Truby (extracted 2026-08-08)

- **Need vs. Desire — never conflate them.** Need is internal: the flaw the character must fix within
  herself. Desire is external: the concrete thing she's consciously pursuing in a given questline. They're
  often connected (achieving the desire can also satisfy the need), but they are structurally two different
  things — collapsing them into one is one of the most common ways a character ends up flat.

- **Give every companion a moral need, not just a psychological one.** A psychological need is a flaw
  hurting only the character herself. A moral need is a flaw actively hurting *someone else*, which she has
  to learn to stop doing. Psychological-only makes a character passive or victim-shaped. A real moral need
  gives her actions genuine consequences for other people — which is exactly the kind of material that makes
  a Dual-Outcome branch feel earned rather than arbitrary (see the Branch-Justification Check, above).

- **Derive the moral need from the psychological need — don't invent it separately.** Two concrete
  techniques: (1) start with the psychological weakness, then ask what harmful action toward others would
  naturally flow out of it; (2) take an established strength or virtue and push it far enough that it curdles
  into the moral flaw — a virtue in excess becoming a vice.

- **The Ghost — the one specific wound, not generic backstory.** Distinct from "backstory" (everything that
  happened to her): the ghost is the single open wound that is the actual *source* of the psychological/moral
  weakness. Structurally it functions as an internal opponent — a counterdesire pulling her backward while
  her present-day desire pulls her forward. Practical technique: withhold the ghost's full detail early and
  reveal it gradually, often through another character naming it rather than the companion stating it
  herself — front-loading it as exposition kills the effect.

- **The character must not already know her own need.** If she's consciously aware of what she needs to fix
  from the start, the arc is already over before it began. The realization belongs at the climax, earned
  through the questline's own struggle — never stated upfront as a settled fact on a spec sheet.

- **The opening problem should be the external symptom of the internal weakness, not a separate hook.** The
  concrete crisis a companion is facing when the player meets her should visibly manifest her core weakness
  — not be an arbitrary plot device bolted on independently of what she actually struggles with.

- **The Character Web — individuate every companion by comparison, never in isolation.** Don't build one
  companion's psychology, then the next's, as separate exercises. List the same six parameters for the whole
  roster side by side — weaknesses, need (psychological *and* moral), desire, values, power/status/ability,
  and how each one faces the same central moral problem — starting with the comparison between the most
  important pair (e.g. a companion and her closest narrative rival/foil), then radiating outward to the rest.
  This is a genuinely different discipline than writing a great character sheet per person: it's what makes
  an entire roster read as one coherent cast rather than several unrelated character studies.

- **Identify vs. empathize — players don't identify with surface traits.** A player doesn't bond with a
  companion because of her job, background, or dress — those details don't transfer, since no two players
  share all of them. Players identify through desire (what she's visibly going after) and need (the moral
  problem she's wrestling with), and *empathize* — understand and care about her — through seeing her
  motive, even when what she does isn't likable. Always show *why* a companion acts as she does. That's
  what keeps a player engaged through an unlikable or morally compromised stretch, not making her likable
  at every moment.

- **True character change is a changed belief leading to new moral action — not a surface change.** Getting
  richer, more skilled, or more confident are not character changes on their own; they're just changes. A
  real arc challenges and changes what a companion actually *believes* about how to live, and that changed
  belief has to cash out in a genuinely different moral action by the end, not just a different circumstance.
  Critically: **the range of that change has to be decided at the *start* of development, not improvised at
  the end** — you can't "flip a switch" in the final scene and call it an arc if nothing about her range of
  possibility was ever set up.

- **A menu of reusable arc shapes**, worth checking a companion's arc against rather than assuming every arc
  is the same shape: Child→Adult (a true coming-of-age is a changed belief + new moral action, never just a
  first-experience milestone), Adult→Leader, Cynic→Participant (pulled from isolation back into caring about
  something beyond herself), Leader→Tyrant (a legitimate, non-shameful direction — forcing others onto your
  own path rather than helping them find their own), Leader→Visionary. Also useful as a register check for
  the arc's *emotional shape*, not just its content: Good-vs-Bad (simplest, hero stays essentially good),
  Tragedy (self-revelation arrives too late to prevent the fall — the "might-have-been" is what generates
  audience sympathy, not victimhood), Pathos, Satire/Irony, Black Comedy.

- **Four-Corner Opposition — a concrete, repeatable technique for roster/rival design.** Take a companion
  and three others who oppose or complicate her (a main rival plus two secondary ones — this maps naturally
  onto companion vs. companion, or companion vs. a district/faction's own competing figures) and "push them
  to the corners": make each one attack the central character's core weakness in a genuinely *different* way,
  put all four in conflict not just with her but with each other, and give each a distinct cluster of values
  (not one value — a related cluster) that collide with the others'. Two disciplines make this actually work:
  look for the positive *and* negative version of the same value in each character (e.g. determined/
  aggressive, honest/insensitive — a companion's strength should also be legible as where she'll go wrong),
  and this pattern is fractal — it can be reapplied at the level of a whole district or faction, not just one
  companion's own immediate relationships.

- **Splitting a shared moral question into oppositions, roster-wide.** Three techniques, usable together:
  (1) work backward from the hero's own final moral decision — the two options she's ultimately choosing
  between are the real thematic core, so identify that decision first and build outward from it; (2) make
  every other major character (companions, rivals, faction figures) a *variation* on the same central moral
  problem, each answering it differently rather than each having an unrelated personal issue; (3) give each
  character a distinct *cluster* of values and put those clusters in direct conflict as they compete for the
  same goal or space. This is the mechanism for making a whole roster feel thematically unified without
  making every character the same — differently-answered, not differently-questioned.

## From *Dynamic Characters* — Nancy Kress (extracted 2026-08-08)

- **What counts as a conflict is itself character-specific, not objective.** The same event is a crisis for
  one person and a shrug for another, purely based on what that specific person values. Before writing a
  companion's reaction to anything, ask what would actually register as a conflict *for her specifically* —
  don't assume a universal threshold.

- **A concrete menu of ways people cope with conflict** — genuinely useful as a checklist to keep a roster's
  reactions from converging on the same two or three responses: denial, rational problem-solving, seeking
  outside allies, turning to professionals, blaming someone else, substance use, going crazy, offloading the
  problem onto someone else, throwing a tantrum, prayer, homicide, becoming depressed and withdrawing,
  giving up/passively accepting fate, forming a committee, placating, punishing whoever points out the
  problem, becoming ill with stress, relying on hope, creating art, trying harder, leaving (the job, the
  relationship, the country), turning pain into helping others, cruelty to the powerless, staying in control
  of everything, learning to avoid the same conflict again, self-sacrifice, negotiation/compromise, suicide,
  covering tracks, revenge, courage. Pick reactions that are *specific* to a companion's established
  psychology, not generically "dramatic."

- **Fear and love as the two deepest generative wellsprings — and you don't need to explain their origin.**
  Buried fear (and its twin, what someone loves enough to fear losing) drives surface behavior more reliably
  than almost anything else. Critically: **you don't have to invent or state the childhood origin of the
  fear in the actual material** — you only need to understand it well enough yourself to make the character's
  *reaction pattern* consistent and convincing. Naming the fear outright in-text often weakens it; a story
  is not a psychological case study. This means a companion's Character Spec doesn't need a fully justified
  origin story for every fear — an internally consistent reaction pattern is enough, and over-explaining can
  hurt more than it helps.

- **Fear and love are frequently the same drive wearing two faces.** Loving something creates fear of losing
  it, and the two are often functionally inseparable — ask whether a companion's protective or self-sacrificing
  behavior is actually driven by love, by fear of loss/failure, or (most interesting) genuinely both at once,
  without the character herself being able to fully separate them.

- **The Preparation → Pressure → Realization → Validation model for character change (distinct from Truby's
  Weakness/Need/Self-Revelation structure — a complementary four-step process, not a competing one):**
  1. **Preparation** — before the change happens, plant evidence the character is *capable* of it: either a
     dormant present-day quality (the person "bad with people" who's unexpectedly good at reading a room),
     or a glimpse of who she used to be before whatever hardened her, shown through memory, an object, or
     another character's testimony. A change that isn't prepared for reads as arbitrary.
  2. **Pressure** — change is threatening, so nobody does it without being forced by something that would
     actually move *this specific* character (concern for someone else, guilt, hitting bottom, love, danger)
     — not a generic amount of narrative pressure, a kind matched to her.
  3. **Realization** — underplay this moment. Don't have the character narrate her own internal shift in
     detail; show a small, understated action or exchange and let the audience deduce the change from it.
     Over-explaining the moment of realization drains its force.
  4. **Validation** — words are never enough, not even the character's own stated self-assessment ("I've
     really changed"). The change only reads as real once it's demonstrated through concrete follow-up
     action after the realization, not at the moment of realization itself.

- **Five shapes for how a companion's actual desire-line can resolve** — useful as a menu distinct from
  Truby's belief-change arc types, since this one is about outcome, not belief: **Victorious** (gets it,
  earns it); **Defeated** (loses it, whether deserved, tragic, or partially compensated by something else
  gained); **Pyrrhic** (gets it, but it costs too much or turns out hollow — pair with a secondary character
  who gets genuine fulfillment, for balance); **Loses by Winning** (gets exactly what she wanted, then
  unconsciously recreates the same old problem inside the new situation — a genuinely sophisticated pattern
...
- **Goal / Desire / Lesson, as "Brain / Heart / Soul"** — Boutros's own vocabulary for the Truby-style
  ...
  writing begins, or "you have a shell, doing things for no particular reason." A companion whose character sheet
  ...
  - **The Lesson doesn't have to be learned, and doesn't have to be positive.** Walter White's arc is Boutros's
    ...
    confirmation when writing character arcs that end badly.
  - **Backstory is inert without a forward-facing Goal.** Boutros flags a specific, common failure mode: a
    ...
    inert biographical color in a character sheet. Depth that doesn't attach to a Goal a companion is actively
...
- **The "Why?" Chain** — a repeatable interrogation technique for finding a Desire underneath a Goal: keep
  ...
  drafting step for any character's Goal/Desire pair — don't stop at the first stated motive.
...
- **Flaw vs. Personality Trait, sharply distinguished.** A **flaw** is a wound-based belief rooted in a
  ...
  unless deliberately rooted in one. The test for whether something belongs in a character's Flaw slot rather than
  ...
  - Also useful: **character trait vs. personality trait**, a second, unrelated distinction Boutros draws —
    ...
    personality traits (bubbly, dour) are outward projection, often deliberately performed for others. A character
...
- **Two concrete Flaw-excavation exercises**, both usable directly as drafting prompts for a character:
...
- **Prime Motivating Incident / Prime Motivating Factor** — St. John's term for what this draft has elsewhere
  ...
  **distorted belief** the character then spends a lifetime either confirming or fighting, not just a fear or
  ...
  and it governs every relationship he has from that point on. Useful framing for a character's core wound: it's
...
- **Compare-and-Contrast Self-Image.** A character's self-image is only meaningful relative to a specific
  ...
  reveals his wound. The generative question: **who or what is this character's yardstick**, and what does she tell
...
- **The "Character Is Not You" caution.** A specific, useful check against writer's-comfort drift: resist
  ...
  specifically for a project with 35+ cities' worth of characters being generated by the same author (and now the
...
- **External Goal / Internal Goal / Greatest Fear, as three distinct and sometimes contradictory layers.** A
  ...
  - **Self-preservation (external) vs. self-protection (internal)**, offered as the two most primal universal
    motivations underneath everything else — a compact axiom worth keeping as a fallback when a character's
...
- **Sacrifice must be scaled to what the specific character actually has, not to objective magnitude.** "A
  ...
  loss," it's "what does *this specific* character actually have that's scarce or precious to her," which will look
...
- **Speech patterns as a diagnostic output of already-established psychology**, a reusable correlation table
  for keeping a character's dialogue consistent with her established wounds rather than drifting into generic
  ...
  others' accomplishments. The discipline: once a character's core wound and self-esteem level are fixed, her
...
- **Dialogue as subtext — a scene can mean something other than what it literally says, and objects/settings
  ...
  character's dwelling, workspace, or personal effects can carry the same characterizing weight as a line of her own
  dialogue.
...
- **Reaction-word choice is itself a characterization decision, not a neutral report of fact.** The specific
  ...
  *screeched* vs. *pleaded* are not interchangeable, and St. John flags that overusing crying specifically as a
  ...
  future writing (dialogue, quest text, companion banter) that narrates a character's reactions: the verb is doing
...
- **"A writer sets up his own amazing experiment: his work of fiction."** Maisel's central framing claim, worth
  ...
  premise of the whole "run a character through test scenarios" approach this file has been assembling piecemeal from
...
- **Characters must be MORE psychologically consistent than real people — this is a feature of the craft, not a
  ...
  confused writing. **Practical implication for character design: depth should come from a rich, well-motivated,
  internally coherent throughline — not from piling on realistic-but-contradictory quirks in the name of
  complexity.** A character can be deep and simple at the same time; she cannot be deep and incoherent. This is worth
  treating as a standing caution on everything else gathered in this file: every technique above should be in
  service of *one* legible, consistent psychology per character, not an ever-growing pile of traits.
  - Directly related, and worth folding in rather than treating as separate: **"Consequences of Upbringing" as
    ...
    convergence as license to keep character wounds single-cause and clean rather than realistically multi-factorial.
...
- **The core mechanism: Scenario + Branching Questions + Interpretation, not open-ended biographical Q&A.**
  ...
  is a technique this project could apply directly and cheaply: for any character whose psychology feels
...
- **The "why is this character eluding me" troubleshooting checklist** — useful as a fast diagnostic when a
  character still won't come into focus after the above techniques: Does she actually have a place in the story, or
...
applying the method to this project's own characters and scenarios, not in the specific real-world scenario text.
...
its own scenario bank natively in Tepenia's own psychology (Robot Universals, character-specific canon, this
project's own design laws) instead of importing 20th-century American scenarios wholesale. Use this to
construct new diagnostic scenarios from scratch for any character whose psychology feels underspecified.
...
5. **For each option, write one interpretive line using "consistent with" language.** Name the underlying
   ...
   to the human in the room without being asked is consistent with a character who was built or spent formative
...
9. **Enforce the consistency discipline when actually answering.** Per Maisel's own core caution (above): the
   options chosen for a single character across every decision point in a scenario — and ideally across every
...
**Worked example** (axis: how a character carries Personality Module tension when preserving a bond costs her
...
*Situation — "Asked to Break Character":* Someone your character has a real bond with — a trust-bonded partner, a
squad-mate, a mentor — asks her, in a moment of genuine need, to do something that runs directly against a
core Personality Module trait: a normally reserved character is asked to be the loud public face of a plan; a
normally blunt character is asked to smooth-talk someone to protect a third party; a normally cautious character is asked
...
- *Decision point 1 — her first reaction, before she's agreed to anything:*
  - She agrees instantly, before she's let herself feel the cost — consistent with a character whose trust-bond
    commitment overrides her own comfort by default, possibly to a fault.
  - She asks "why me" before answering — consistent with a character who needs the request to make sense on its
    own terms before she'll override herself for it.
  - She feels a flash of resentment she immediately suppresses — consistent with a character who's done this kind
  ...
  - She says no, and offers an alternative that doesn't require her to break character — consistent with a
    character whose sense of self is not negotiable even for someone she loves, for better or worse.
  - She's flattered to be asked, mistaking the request for evidence she's trusted with something hard —
    consistent with a character whose self-worth is still tangled up in being useful rather than being herself.
...
- **Strength, Longing, and Fear as the three specific things a character's introduction must establish — a distinct
...
- **Systematically placing "shadow" material at specific structural locations, not just having it present
  ...
  flaw or a hidden skill, and in imagery. Useful as a placement checklist: rather than trusting a character's shadow
...
- **The Thematic Significance Statement construction technique.** A concrete, generative device for turning a
  ...
  revenge and destruction"*). The discipline is refusing to leave a character's or a district's core theme as a bare
...
### Core Vocabulary (the foundation — apply to every character)

- **The Lie.** A single-sentence, specific false belief a character holds about herself or the world, statable in
...
- **Want vs. Need.** The Want is the external, tangible, plot-level goal — concrete, pursuable, often
  ...
  usually **precludes or requires sacrificing the Want**, at least until the character has earned the right to have
  both. The sharp, useful warning here: pursuing the Want, on its own, actively *deepens* enslavement to the
  Lie rather than resolving it — a character chasing her Want without addressing her Need is not neutral, she's
...
- **The Ghost.** The backstory wound that explains *why* the character believes the Lie — "every effect must have a
...
- **The Normal World as symbolic dramatization of the Lie**, not just a setting. The opening world itself
  ...
  continue its existence," whether or not it's comfortable for the character herself (she can be outwardly fine and
...
- **The Moment of Truth, at the story's Midpoint.** A precise, genuinely distinct beat from the Black Moment
  (St. John, above) — this is the *middle* turning point, not the low point. Here the character subconsciously
...
- **Negative Change Arc — three distinct, precisely-defined variations**, directly serving this project's
  ...
  Ghost differs** (falls prey to it vs. overcomes it), meaning a single fully-developed character sheet can honestly
  ...
  - **"Whatever he chooses, he will have to sacrifice something of great value. Likewise, whatever he chooses,
    ...
    a character or district faction faces — both options must be genuinely costly *and* genuinely rewarding, not one
...
- **The 3-question arc-selection method** — a fast decision procedure for picking which arc (and which Negative
  Arc variant) actually fits a given character or companion concept: (1) What's the genre/tone requirement — does
...
- **The Impact Character.** A formally named *function*, not a character-type slot — the person (or persons)
  ...
  portable: when designing any character's arc, explicitly ask *who or what is her Impact Character* — and don't
...
- **Rewards and Punishments as the mechanical engine of change.** A concrete, almost mechanical technique for
  ...
  Early on, Lie-based choices should lead to close-to-full disasters; as the character begins acting more in line
...
generalizes across every future character — has already been captured, this is judged sufficient without a complete
...
- **Negative endings are one option among several, not the default or the majority.** For companion and
  ...
  or at most a handful, of the total possible outcomes for any given character; **bittersweet/mixed endings should
...
- **This project's existing "No Good Endings" law does *not* mean "no positive endings."** It means no
  *costless* positive endings. A true positive resolution must still cost the character (and ideally the player too)
...
- **The specific mechanic for a genuine positive ending, as the dev described it: the character (and ideally the
...
- **Practical implication for future character-arc design**: when drafting a companion's set of possible endings,
...
- **This entire principle scales up unchanged — the same law governs district, DLC, and civilizational-level
  ...
  cost that district/faction/subnet something real and named — not just an individual character — in order to gain
...
placing a given character's current arc within a larger life-progression frame: **First Act/Individuation** (Maiden
...
than inventing a Lie from scratch for every character, start by checking whether one of these six already fits her
life-stage:
...
character doesn't complete an arc and then stay in it forever — completing it is itself the setup for eventually
...
simply a separate person to be saved — a useful reminder when designing a Hero-arc character's own supporting cast:
...
- **Resolution (Kingdom Renewed for Another Cycle):** he departs, in sorrow but without regret, having "fought
  ...
  questline in its own right; treat it as the least likely of the six to be a full dedicated arc for a character, and
...
- **Snow Queen (Queen's passive shadow):** may have gone through the *motions* of a completed Hero Arc —
  ...
  romanceable Snow-Queen-type character.
...
- **Sorcerer (Mage's aggressive shadow):** the sharpest, most portable single distinction in the whole shadow
  ...
  true name." A clean single-axis test worth applying to any character's use of power or knowledge: is she reaching
...
- **The two poles are latent within each other, not separate people.** *"Inherent within any Coward is usually
  ...
  shadow-state character: her surface presentation (passive or aggressive) shouldn't be treated as her whole
...
- **A calibration rule for which negative archetypes a Flat companion can plausibly engage with**: a Flat
  ...
  character actually need her own Change Arc after all" — if her questline is pushing her up against a shadow-stage
...
- **Child (precedes Maiden).** Represents raw, untapped potential rather than any Truth actually learned yet —
  ...
  back toward their own lost openness. Directly useful for a very young or newly-activated character: she doesn't
...
- **Parent (between Hero and Queen).** The Hero's "happily ever after," made literal: stable, content, focused
  ...
  primary narrative utility is almost always in relation to a specific younger character she's raising or has raised,
...
- **Ruler (between Queen and King).** The height of a person's power, and — distinctively — someone who has
  ...
  given character: does she primarily *love and protect* her charges (Parent), or does she primarily *impose healthy
...
- **Mentor (between Crone and Mage).** Has already completed the Crone Arc and made a real, hard-won peace with
  ...
  remembering when casting a Mentor-type character: her presence in a scene should itself signal that something
...
1. **Identify which archetype is actually already present, rather than picking one abstractly first.** Two
   valid directions: start from an archetype and build a character to fit it, or — usually more productive when a
...
3. **Use the protagonist's chosen arc to select and populate her supporting cast.** The two-slot checklist
   ...
   character's arc is fixed, actively look for which specific Flat archetype and which specific antagonist pairing
...
4. **Let the chosen archetype's built-in theme actually govern word and scene choice**, not just the big Lie/
   ...
   King's strength-vs-surrender, and so on, already captured above) — once an archetype is chosen for a character,
...
5. **Draw on the designer's own lived experience of these same life stages as a resource, not just an abstract
   ...
   writing a character at that same stage, since the archetypal pattern is something every person actually goes
   through, not just a structure imposed on fiction from outside.

**Standing caveats worth keeping alongside the five steps:**
- **The system doesn't have to be applied literally or in strict sequence.** A character's story can use archetypal
...
- **This is one archetype system among many, not exhaustive or the only valid lens.** Named archetype-types
  ...
  others) remain legitimate tools in their own right for a character whose psychology doesn't map cleanly onto any
...
- **The New Normal World is defined by the character's *changed relationship* to her surroundings, not by a
  ...
  5. **A natural confirmation that it's time to leave permanently** — the ending isn't a return at all, but a
     deliberate departure (a coming-of-age character moving on to her next arc/DLC).
  - **Never physically leaving one setting for the whole story still supports this framework** — the "worlds"
    don't require literal travel; a character who stays in one city or district the entire time can still pass
...
- **Practical workflow note, matching (and specifically justified by) advice already captured elsewhere in
  ...
  first — it's described as the single scene that reveals what a given story (or character's arc) is actually
...
- **Characters We Love / Characters We Hate** (Chapter 8) — the single richest chapter in the book: a working
  ...
  - *Antipathy levers*: the **Sadist/Bully** distinction (the defining trait isn't a love of causing pain, it's
    ...
    **Assassin vs. Avenger** (violence and even killing are not automatic antipathy triggers — motive and the
    ...
    calibrating District power-grab antagonists and any character or human figure whose authority in a questline is
...
- **Wild Cards — a two-tier taxonomy for offbeat/deviant characters** (Chapter 9), useful for both minor
  ...
  project's own practice of treating a character's Enneagram type as fixed across her arc even as wing, subtype, or
  health-level shift.
...
- **"Actable" description** (Chapter 2) — a screenwriting-specific concept with a portable core: a description
  ...
  notes: prefer describing a character's default posture, characteristic gesture, or movement quality over adjective-
